...
- 必须荒诞、离谱、有梗、适合做成表情包
- 禁止使用正常、平淡、无聊的表达
- 可以参考：禁止蕉绿、汗流浃背了吧、发疯冠军、情绪稳定、已黑化、在疯了别惹我、精神状态良好、今日歇业
- 禁止使用意义不明的过气的梗：'尊嘟假嘟'、'泰酷辣'
- 字数控制在 3-7 字，越离谱越好
...
# JSON Output Format
{
  "status_label": "抽象状态命名（基于status_tag自由发挥，更加个性化和网感化）",
...
## status_label 特别说明（关键！）
status_label 不是枚举，而是基于 status_tag 自由生成的抽象状态命名：
- **不要**直接复制 status_tag 的值
- **要**根据 status_tag 的含义，创造更有网感、更个性化的表达
- 可以结合 scene 和 style_mode 来增加画面感
- **重要：每次生成不同的表达，避免重复使用相同的比喻或隐喻**

**多样性要求**：
每次调用时，请从以下不同角度中选择一个来创造status_label：
1. **电量/能源类**：电池、CPU、电量、充电、续航等隐喻
2. **系统/软件类**：系统、进程、后台、死机、卡顿等隐喻
3. **游戏/状态类**：HP、技能、冷却、红蓝条、buff/debuff等隐喻
4. **物理/力学类**：惯性、摩擦力、重力、阻力、势能等隐喻
5. **天气/自然类**：阴天、雾霾、低气压、无风带、冰冻等隐喻
6. **职场/社会类**：离职、摆烂、内卷、躺平、摸鱼等隐喻

**优秀示例**（基于不同的 status_tag 和角度）：
- "疲惫/低启动" → 
  - 电量类："电量只剩1%的赛博游魂"、"CPU降频中请勿打扰"
  - 系统类："系统休眠模式已启动"、"后台进程全部卡死"
  - 游戏类："HP见底的红血状态"、"蓝量耗尽无法施法"
  - 物理类："惯性滑行中无动力"、"摩擦力过大无法启动"
  - 天气类："无风带的停滞船只"、"低气压抑郁天气"
  
- "拖延/回避" → 
  - 电量类："薛定谔的努力（观测前不存在）"
  - 系统类："明日复明日.exe已停止响应"
  - 游戏类："任务接取成功（但不做）"
  - 职场类："精神离职状态已开启"、"职场隐身模式激活"
  
- "羞耻/自我否定" → 
  - 系统类："自我PUA后台进程占用100%"
  - 游戏类："Debuff叠加到上限"、"自我攻击技能满级"
  - 职场类："内心戏奥斯卡最佳提名"、"精神内耗KPI已达标"
  
- "轻度发疯/情绪过载" → 
  - 系统类："情绪稳定地发疯"、"系统报错但继续运行"
  - 游戏类："狂战士模式已开启"、"理智值归零暴走中"
  - 天气类："情绪台风眼中心"、"精神海啸预警中"
  
- "想做但卡住" → 
  - 系统类："卡在了开始的开始"、"启动失败请重试"
  - 游戏类："99%进度条卡住中"、"加载中（永远）"
  - 物理类："静摩擦力大于推动力"、"势能被势能井困住"`;
